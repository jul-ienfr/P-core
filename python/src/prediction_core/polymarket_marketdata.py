...
def replay_clob_ws_events(events: list[dict[str, Any]], *, cache: MarketDataCache | None = None) -> dict[str, Any]:
    ...
    for index, event in enumerate(events):
        _apply_clob_ws_event(event, cache=target_cache, stats=stats, index=index)
...
async def run_clob_marketdata_stream(
    *,
    ...
    async for event in stream_factory(url, subscribe_message):
        if max_events is not None and stats["received_events"] >= max_events:
            break
        stats["received_events"] += 1
        _apply_clob_ws_event(event, cache=target_cache, stats=stats, index=stats["received_events"] - 1)
...
def _apply_clob_ws_event(
    ...
    *,
    ...
    token_id = event.get("asset_id") or event.get("token_id") or event.get("token")
    try:
        cache.update_book(
