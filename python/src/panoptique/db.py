...
DEFAULT_SYNC_DATABASE_URL = "postgresql+psycopg://panoptique:panoptique@localhost:5432/panoptique"


def get_database_url(*, required: bool = True) -> str:
    url = os.environ.get("PREDICTION_CORE_DATABASE_URL") or os.environ.get("PANOPTIQUE_DATABASE_URL")
    if url:
        return url
    if required:
        raise RuntimeError(
            "PREDICTION_CORE_DATABASE_URL or PANOPTIQUE_DATABASE_URL is required for DB writes; "
            "copy infra/storage/.env.example or infra/panoptique/.env.example for local development."
        )
    return DEFAULT_ASYNC_DATABASE_URL


def get_sync_database_url(*, required: bool = False) -> str:
    url = os.environ.get("PREDICTION_CORE_SYNC_DATABASE_URL") or os.environ.get("PANOPTIQUE_SYNC_DATABASE_URL")
    if url:
        return url
    async_url = os.environ.get("PREDICTION_CORE_DATABASE_URL") or os.environ.get("PANOPTIQUE_DATABASE_URL")
    if async_url:
        return async_url.replace("postgresql+asyncpg://", "postgresql+psycopg://", 1)
    if required:
        raise RuntimeError("PREDICTION_CORE_SYNC_DATABASE_URL or PANOPTIQUE_SYNC_DATABASE_URL is required for sync DB operations")
