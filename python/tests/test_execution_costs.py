...
def test_build_execution_cost_breakdown_public_api_uses_fair_probability_edge() -> None:
    book = OrderBookSnapshot(
        bids=[BookLevel(price=0.42, quantity=100.0)],
        asks=[BookLevel(price=0.45, quantity=10.0), BookLevel(price=0.46, quantity=15.0)],
    )

    result = build_execution_cost_breakdown(
        book=book,
        requested_quantity=20.0,
        side="buy",
    ...
    assert result.book_slippage_cost == 0.1
    assert result.trading_fee_cost == 0.0182
    assert result.deposit_fee_cost == 1.0
    assert result.withdrawal_fee_cost == 2.0
    assert result.total_execution_cost == 0.4182
    assert result.total_all_in_cost == 3.4182
    assert result.edge_gross == 0.9
    assert result.edge_net_execution == 0.4818
    assert result.edge_net_all_in == -2.5182
