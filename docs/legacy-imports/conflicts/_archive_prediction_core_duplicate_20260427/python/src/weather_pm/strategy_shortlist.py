...
from prediction_core.strategies.weather_bookmaker import build_weather_bookmaker_signal
...
def _shortlist_row(
    ...
    *,
    ...
    weather_bookmaker_signal = _weather_bookmaker_signal(
        opportunity,
    ...
    return {
        ...
        "weather_bookmaker_signal": weather_bookmaker_signal,
...
def _weather_bookmaker_signal(
    ...
    *,
    ...
    probability = _optional_number(opportunity.get("prediction_probability"))
    if probability is None and entry_decision is not None:
        probability = _optional_number(entry_decision.get("model_probability"))
    market_price = _optional_number(opportunity.get("market_price"))
    if market_price is None and entry_decision is not None:
        market_price = _optional_number(entry_decision.get("market_price"))
    edge = _optional_number((edge_sizing or {}).get("net_edge"))
    if edge is None and entry_decision is not None:
        edge = _optional_number(entry_decision.get("edge_net_all_in"))
    if edge is None:
        edge = _optional_number(opportunity.get("probability_edge"))
    spread = _optional_number(opportunity.get("spread"))
    depth = _optional_number(opportunity.get("order_book_depth_usd"))
    payload = {
        "market_id": str(opportunity.get("market_id") or ""),
        ...
        "profitable_wallets": {
            ...
            "alignment": "yes" if matched_accounts else "none",
        },
        "event_surface": {"inconsistency_count": len(inconsistencies)},
        "execution": {
            "spread": spread,
            "depth_usd": depth,
            "fillable_size_usd": depth,
            "best_effort_reason": _execution_blocker(opportunity),
        },
        "resolution": {
            "source_direct": bool(opportunity.get("source_direct")),
            "provider": opportunity.get("source_provider"),
            "station_code": opportunity.get("source_station_code"),
            "source_latest_url": opportunity.get("source_latest_url"),
        },
    }
    return build_weather_bookmaker_signal(payload).to_dict()
