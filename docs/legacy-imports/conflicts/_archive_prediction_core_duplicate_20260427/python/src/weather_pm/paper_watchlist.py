...
def build_paper_watchlist_report(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    as_of = _parse_iso_date(payload.get("as_of_date") or payload.get("market_date") or payload.get("generated_at"))
    ...
    for position in normalized_positions:
        ...
        if _market_date_elapsed(enriched_position, as_of=as_of) and row["operator_action"] not in {"EXIT_PAPER", "RESOLUTION_REVIEW"}:
            row["operator_action"] = "RESOLUTION_REVIEW"
            row["reason"] = "market date elapsed; verify official resolution before final PnL"
            row["add_allowed"] = False
            row["max_add_usdc"] = 0
    ...
    return {
        "summary": {
            ...
            "active_positions": sum(1 for row in watchlist if row["operator_action"] not in {"EXIT_PAPER", "RESOLUTION_REVIEW"}),
            "resolution_review_positions": action_counts.get("RESOLUTION_REVIEW", 0),
            "exit_paper_positions": action_counts.get("EXIT_PAPER", 0),
            ...
            "total_mtm_bid_value": round(sum(float(row["paper_mtm_bid_value_usdc"] or 0.0) for row in watchlist), 2),
...
def build_paper_watch_row(
    ...
    *,
    ...
    filled_usdc, shares, entry_avg = _reconciled_paper_economics(position)
    ...
    paper_mtm_value = round(shares * best_bid, 3) if best_bid is not None else None
    paper_mtm = round(paper_mtm_value - filled_usdc, 3) if paper_mtm_value is not None else None
    ...
    return {
        ...
        "paper_mtm_bid_value_usdc": paper_mtm_value,
...
def _build_paper_watchlist_exposure_index(positions: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    total = 0.0
    for position in positions:
        amount, _, _ = _reconciled_paper_economics(position)
        if amount <= 0.0:
            continue
...
def _reconciled_paper_economics(position: dict[str, Any]) -> tuple[float, float, float]:
    """Return spend, shares, entry avg using fill batches as the source of truth when present."""
    batches = position.get("batches") if isinstance(position.get("batches"), list) else []
    batch_spend = 0.0
    batch_shares = 0.0
    for batch in batches:
        if not isinstance(batch, dict):
            continue
        spend = _paper_float(batch.get("usdc", batch.get("filled_usdc", batch.get("spend_usdc"))), default=None)
        shares = _paper_float(batch.get("shares"), default=None)
        if spend is None or shares is None or spend <= 0.0 or shares <= 0.0:
            continue
        batch_spend += spend
        batch_shares += shares
    if batch_spend > 0.0 and batch_shares > 0.0:
        return round(batch_spend, 4), round(batch_shares, 4), round(batch_spend / batch_shares, 4)

    spend = _paper_float(position.get("filled_usdc", position.get("spend_usdc")), default=0.0) or 0.0
    shares = _paper_float(position.get("shares"), default=0.0) or 0.0
    entry_avg = _paper_float(position.get("entry_avg"), default=None)
    if entry_avg is None:
        entry_avg = spend / shares if shares else 0.0
    return round(spend, 4), round(shares, 4), round(entry_avg, 4)
...
def _market_date_elapsed(position: dict[str, Any], *, as_of: date | None) -> bool:
    if as_of is None:
        return False
    market_date = _parse_market_month_day(position.get("date"), year=as_of.year)
    return market_date is not None and market_date < as_of


def _parse_market_month_day(value: Any, *, year: int) -> date | None:
    if value is None:
        return None
