...
from .weather_bookmaker import WeatherBookmakerStrategy
...
WEATHER_BOOKMAKER_FIXTURE = {
    "market_id": "fixture-weather-bookmaker",
    "market_price": 0.56,
    "weather": {"probability_yes": 0.64, "confidence": 0.72, "edge": 0.08},
    "profitable_wallets": {"matched_count": 1, "alignment": "yes"},
    "event_surface": {"inconsistency_count": 1},
    "execution": {"spread": 0.02, "depth_usd": 250.0, "fillable_size_usd": 80.0},
    "resolution": {"source_direct": True, "provider": "fixture"},
}
...
def strategy_smoke_summary() -> dict[str, object]:
    ...
    registry.register(WeatherBookmakerStrategy(payloads=[WEATHER_BOOKMAKER_FIXTURE]), StrategyConfig(strategy_id="weather_bookmaker_v1", enabled=True))
