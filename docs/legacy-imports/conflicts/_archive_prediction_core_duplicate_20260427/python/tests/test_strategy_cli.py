...
def test_strategy_smoke_summary_fixture_safe() -> None:
    ...
    assert {item["strategy_id"] for item in summary["strategies"]} == {"weather_baseline", "panoptique_shadow_flow", "weather_bookmaker_v1"}
    assert all("recommendation" not in result for result in summary["results"])
    assert sum(result["signal_count"] for result in summary["results"]) == 3
    bookmaker = next(item for item in summary["strategies"] if item["strategy_id"] == "weather_bookmaker_v1")
    assert bookmaker["mode"] == "paper_only"
...
def test_cli_main_prints_json(capsys) -> None:
    ...
    assert [item["mode"] for item in payload["strategies"]] == ["paper_only", "paper_only", "research_only"]
